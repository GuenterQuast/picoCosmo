...
def pdf(t, fbg=0.1, tau=2.2, a=1.0, b=9.75):
...
if largv >= 2:
    ...
    dtmax = 15.0  #
    if largv >= 3:
        dtmin = float(sys.argv[2])
    if largv >= 4:
        dtmax = float(sys.argv[3])
else:
    infile = 'dpKanne.dat'
...
tauVals = np.linspace(1.0, 4.0, 300)
...
axh.text(0.6, 0.93, r'$\tau=$' + '%.2f' % (tau) + r'$\mu$s', transform=axh.transAxes)
axh.text(0.6, 0.85, 'UG-Anteil= %.2f' % (bg), transform=axh.transAxes)
