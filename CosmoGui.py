...
class Ui_CosmoWindow(object):
    ...
    def retranslateUi(self, CosmoWindow):
        ...
        self.pB_SaveDefault.setText(_translate("CosmoWindow", "Save Config"))
...
# initialization dependent on DAQ config file
    ...
    def initDAQ(self, DAQconfFile):
      ...
      if "DeviceFile" in DAQconfdict: 
        self.PSfile = DAQconfdict["DeviceFile"] # configuration file for scope
      else:
        self.MB_Warning('Warning', 
      ...
      if "BMfile" in DAQconfdict: 
        self.BMfile = DAQconfdict["BMfile"] # Buffer Manager configuration file 
      else:
        self.MB_Warning('Warning', 
      ...
      if "PFfile" in DAQconfdict: 
        self.PFfile = DAQconfdict["PFfile"] # Buffer Manager configuration file 
      else:
        self.MB_Warning('Warning', 
   ...
   # display config data in GUI
      self.pTE_OsciConfig.setPlainText(open(path + '/' + self.PSfile, 'r').read() )
      self.pTE_BMconfig.setPlainText(open(path + '/' + self.BMfile, 'r').read() )
      self.pTE_PFconfig.setPlainText(open(path + '/' + self.PFfile, 'r').read() )
...
# set font for plainTextEdit to monospace
    ...
    def saveConfig(self, confdir):
        ...
        try:
          ...
          self.MB_Warning('Warning', 
             'PS config is not valid yaml format')       
          return 1
        ...
        # generate a DAQ configuration file
        DAQconf = 'DeviceFile: ' + self.PSfile + '\n' +\
                  'BMfile: ' + self.BMfile + '\n' +\
                  'PFfile: ' + self.PFfile + '\n'
